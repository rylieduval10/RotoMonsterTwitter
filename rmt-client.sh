#!/usr/bin/env bash
#
# RotoMonsterTwitter - build out the Client project: a typed wrapper over the
# API endpoints. Posting is stubbed pending Ken's answer on credentials.
#
...
==================================================================
Done. Usage from any caller:

  var client = new RotoMonsterTwitterClient("http://localhost:5080");

  var recent = await client.GetTweetsBySportAsync(1, 10);
  var one    = await client.ReadTweetAsync("2080330099069759891");
  var purged = await client.DeleteTweetsOlderThanAsync(
                   DateTime.UtcNow.AddDays(-30));

Or, in an ASP.NET app:

  builder.Services.AddHttpClient<IRotoMonsterTwitterClient,
                                 RotoMonsterTwitterClient>(c =>
      c.BaseAddress = new Uri("https://twitter.rotomonster.com/"));

Every method returns a result object rather than throwing - network
and HTTP errors come back as Success=false with ErrorMessage set.
==================================================================
MSGEOF
